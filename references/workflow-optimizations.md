...
Cross-model review uses whichever model the user confirms — there is no fixed family mapping and no env pin that skips the question. Before dispatching, list currently available models from the review harness catalog (e.g. `opencode models`), suggest the strongest suitable model that is independent of the authoring model (from that live list only — never a remembered or pinned ID), and ask via `AskUserQuestion` whether to use the suggestion, pick another catalog entry, or skip. Do not dispatch without an explicit choice. Run the selected reviewer once on a frozen diff before ship in read-only/plan mode, follow the command's neutral-prompt rule, and record the resolved model ID and harness. Triage findings as `BLOCKER`, `IMPORTANT`, or `NIT`; fix confirmed `BLOCKER`s in one batch. Do not re-run after every micro-fix. Re-run only after a material scope change or an unresolved correctness question. `IMPORTANT` and `NIT` findings do not block shipping unless the user promotes them.
