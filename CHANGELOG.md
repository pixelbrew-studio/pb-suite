...
- `/pb-ship` follows the merge commit after the ship decision (step 7b, `--no-follow` opts out). A green PR is not a shipped change: the command tracks the exact merge SHA through CI, GitHub deployment status, and an optional `## pb-suite: health check` URL declared in `CLAUDE.md`. It tracks the SHA rather than the branch because on an active base branch "the latest run" is routinely another commit, and it treats a `cancelled` run as a distinct outcome — a superseded run means the commit never deploys on its own, so the command confirms via `git merge-base --is-ancestor` that the change survives into the new head and retracks that SHA. Repos with no deployment record skip that stage instead of failing.

- New `/pb-handoff`. `/pb-resume` reconstructs a repo from git; this records what git never held — why a decision went the way it did, and what was already tried and abandoned. Two rules carry it: state rather than instructions (a handoff of imperatives makes the next agent execute a plan it cannot evaluate), and reference rather than duplicate (a handoff that copies its sources goes stale silently, and stale context is worse than none because it gets trusted). Writes `.context/handoff.md`, or `HANDOFF.md` with `--repo`. Carries forward the uncertain calls `/pb-decisions` surfaces, and closes with an instruction to verify every claim against the code rather than accept it.
- Opt-in command guard, `./install --hooks`. `/pb-review`, `/pb-pr`, and `/pb-ship` already refuse `--no-verify` and `--force`, but that is compliance asked of the model; this is a `PreToolUse` hook that exits 2 before the command runs. Blocks the irreversible only — `rm -rf` at `/`, `~`, or `/Users`, raw-disk writes, `sudo rm`, `curl | sh`, `git push --force`, remote branch deletion, reflog destruction, `gh repo delete`, `gh auth token` — while `rm -rf node_modules`, `--force-with-lease`, and `git gc --prune=2.weeks.ago` stay allowed, because a guard that blocks routine work gets switched off. Fails open without `jq` or a patterns file, for the same reason. It is a seatbelt against an accident, not a sandbox: an obfuscated equivalent walks past a regex denylist. The settings merge backs up `settings.json`, preserves unrelated hooks, and is idempotent; uninstall removes only pb-suite's entry. Codex wiring stays manual and documented, since Codex pins hook trust by hash and needs an interactive re-trust. Adapted from davidondrej/skills (MIT); see `NOTICE`.
- Codex skills that are deliberate user-invoked steps now ship `agents/openai.yaml` with `allow_implicit_invocation: false` — `/pb`, `/pb-across`, `/pb-decisions`, `/pb-evolve`, `/pb-handoff`, `/pb-init`, `/pb-pr`, `/pb-resume`, `/pb-roast`, `/pb-rules`, `/pb-ship`. Claude Code's `disable-model-invocation` is not part of the Agent Skills spec, so without this file Codex could fire a merge gate or a suite-editing command off the shape of a change. Trigger-table commands stay implicitly invocable and get no file.
- Fix invalid YAML frontmatter in the `pb-ship` Codex skill. Its unquoted `description` contained `: `, which a strict YAML parser rejects — Claude Code's lenient parser accepted it, so the break was invisible from the Claude side while the Codex skill could silently fail to load. A smoke assertion now parses the frontmatter of every command and Codex skill, so the class of bug cannot return unnoticed.
