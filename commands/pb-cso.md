...
- **A01 Broken Access Control** — every authenticated route enforces auth (middleware, decorator, or in-handler check)? Are user-id-bearing inputs verified against the session subject (no `req.body.userId` trust)? IDOR on REST collections (`/users/:id`)? Open-redirect and string-classifier validation — are redirect targets (`returnTo`/`next`), route classifiers (auth gates, PII/allowlist routers), and slug→path lookups checked against the canonical parser (resolve, compare `.origin`) or an anchored pattern, not hand-rolled prefix/substring checks that miss `/\evil` → off-origin, `/legalese` prefix-matching `/legal` without a `(?:\/|$)` boundary, or `..`/encoded path traversal? Test the adversarial near-miss, not the happy path. Multi-tenant RLS write paths — multiple permissive policies OR-combine, so an UPDATE can move a row across scopes unless `WITH CHECK` pins the same scope as `USING` (or ownership columns are immutable); every tenant-bearing FK on a child/join table needs its own `owns_*` check in `WITH CHECK` (FK existence is not a tenancy check); INSERT-time consistency triggers are bypassable by a later parent-FK UPDATE unless parentage columns are immutable. Test UPDATE scope-crossing and cross-tenant FK inserts, not just SELECT isolation.
...
- **A04 Insecure Design** — rate limiting on auth and payment endpoints? Account-enumeration via login error messages or password-reset response timing? Idempotency on payment webhooks? On credit/spend paths — is the whole region between the spend and the terminal response guarded? A catch there must refund/restore state or durably record the stuck id, never just classify the error; retry/reconcile layers must key on the downstream operation's idempotency key, with idempotency verified per backend the retry can actually reach.
