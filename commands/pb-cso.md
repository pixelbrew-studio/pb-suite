...
Some projects declare a canonical narrative in `CLAUDE.md` — a claim every external surface must make concrete (e.g. Evalgist's privacy-first / non-retention / GDPR posture, or a competitor's "no-data-sold" posture). When the diff adds or modifies copy on an external surface, that surface must back the claim with at least one concrete, verifiable statement — not a "we take X seriously" cliché.
