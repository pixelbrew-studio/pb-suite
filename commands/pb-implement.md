...
Workflow: `/pb-implement <what>` → spec + verification plan → (human gate if strict) → `/pb-tdd` loop per slice (cheaper capable sub-agents for bounded bulk work, strongest model for critical work) → verify against plan → `/pb-check` (suite review breadth) + suggested-and-confirmed independent cross-model review (depth) → learning artifacts → `/pb-pr`/`/pb-ship`.
...
**8b. Cross-model depth.** Tests and same-model review both inherit the author's blind spots. Cross-model review is deliberate model-family independence — but the reviewer is not auto-selected.

**Pick the model first.** List currently available models from the review harness catalog (e.g. `opencode models`). From that live list only — never a remembered ID, env pin, or stale default — suggest the strongest suitable model that is independent of the authoring model (same family as the session that wrote the diff does not satisfy the gate), with a one-line reason. Ask via `AskUserQuestion`: use the suggestion, pick another catalog entry, or skip. Do not dispatch without an explicit choice; do not honour `PB_CROSS_MODEL_REVIEW_MODEL` or any other pin that bypasses the question.

Confirm the chosen ID exists in the catalog. If it does not, report and re-ask; do not substitute another model silently. If the user declines or no model is chosen, say so in the report; the cross-model pass remains unavailable and strict work remains blocked.

Use a read-only/plan mode, and record the exact resolved model ID and harness. Dispatch the review of the branch diff (vs the merge base) with the spec (`.context/spec-<slug>.md`) and risk bucket as context. Independence from the agents that wrote *and* reviewed the slices is the point.
...
- **Silent cross-model pick.** Do not map the authoring model to a fixed reviewer family, dispatch without asking, or honour env pins that skip the question. List the live catalog, suggest the strongest suitable independent model with a reason, get explicit confirmation, then record the resolved model ID.
