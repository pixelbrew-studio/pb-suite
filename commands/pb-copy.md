...
**Monorepo content-style packages** (e.g. Evalgist's `@evalgist/content-style`). Look for packages whose name or path hints at voice/tone/brand/copy:
...
- **Product-naming rules** — first-mention vs second-mention, sub-brand bans, deprecated names (e.g. Evalgist's "hireIQ" → "Evalgist Shortlist")
